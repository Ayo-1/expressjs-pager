...
module.exports = function(options) {
	...
	if(options.template && options.template.ul && options.template.li && options.template.a && options.template.next && options.template.previous){
		var customDir = path.join(__dirname, '../views');
		var viewsDirectory = path.join(__dirname, '../views');	
		var templates = {
			pagination: path.join(customDir, 'cpagination.pug'),
			page: path.join(customDir, 'cpage.pug'),
			next: path.join(customDir, 'cnext.pug'),
			previous: path.join(customDir, 'cnext.pug'),
	...
	function pager(itemCount, itemsPerPage, current) {
		...
		return {
			pages: pages,
			render: function render(r_options) {
				...
				var html = fn({
					baseUrl: r_options.baseUrl || '/',
					pages: pages,
					currentPage: currentPage,
					query: r_options.query? get_q(r_options.query) : "",
